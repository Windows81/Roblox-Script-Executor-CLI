...
class exec_processor:
    ...
    def parse(self, input_gen=_gen(input)):
        ...
        # Multi-line script.
            ...
            append_block = f"T[I]={self.parse_str(rep_cmd)}"
        ...
        elif head_l in ["f", "function"]:
            arg_t = f"local A={{...}}"
            arg_n = f"local {','.join(f'A{n}' for n in range(1,10))}=..."
            return f"(function(...)\n{arg_t}\n{arg_n}\n{self._param_single(body)}\nend)"
    ...
    def parse_str(self, s):
        return self.parse((_ for _ in [s]))
